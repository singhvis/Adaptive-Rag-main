...
if "session_id" not in st.session_state:
    ...
    if token:
        ...
        # Local fallback when external auth service is unavailable.
        st.session_state["session_id"] = "local-dev-session"
        st.session_state["jwt_token"] = "local-dev-session"
        st.warning(
            "Auth service unavailable at localhost:8080. "
            "Running in local dev mode without login."
        )
